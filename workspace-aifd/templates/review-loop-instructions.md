...
当 `workspace/pipeline.json` 中 `is_incremental: true` 时，Review Loop 进入增量模式：

### 文档路径
- 产出文档写入 `docs/specs/features/{feature_id}/` 目录（而非全量 `docs/specs/`）
- 自检和评审时，同时参考全量 specs 确保一致性

### 评审团额外关注点
**需求文档评审**：
- 增量需求是否与全量 requirements.md 矛盾？术语和角色定义是否一致？
- 增量边界是否清晰（新增什么、修改什么、不涉及什么）？

**产品设计评审**：
- 增量页面/交互是否与全量导航结构、交互风格兼容？
- 对现有页面的改动是否明确标注？

**技术设计评审**：
- 增量设计是否与全量架构分层、数据模型、API 风格一致？
- 如涉及数据库 Schema 变更，是否有向前兼容的迁移方案？

### 调度评审团时的 prompt 补充
在调度各评审方时，额外传入：
```
本次评审的是增量特性文档（特性ID: {feature_id}）。
请在评审时同时参考全量文档（docs/specs/），重点检查增量内容与全量设计的一致性和兼容性。
```
